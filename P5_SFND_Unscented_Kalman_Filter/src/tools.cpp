...
VectorXd Tools::CalculateRMSE(const vector<VectorXd> &estimations,
	...
	for(unsigned int i=0; i < estimations.size(); ++i){
		...
		double vx_est = estimations[i][2];
		double vy_est = estimations[i][3];

		double vx_gt = ground_truth[i][2];
		double vy_gt = ground_truth[i][3];		
		
		// if(fabs(vx_est - vx_gt) > 0.97 || fabs(vy_est - vy_gt) > 0.7)
		// {
		// 	std::cout << i <<  "\nestimations : \n" <<  estimations[i] << "\n";
		// 	std::cout << i <<  "\nground_truth : \n" << ground_truth[i] << "\n";
		// }
